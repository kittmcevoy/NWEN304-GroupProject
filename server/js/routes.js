...
module.exports = function (app, path, passport, upload) {
    ...
    app.post('/sendForgotPasswordEmail', upload.single('image'), async(req, res) => {
        ...
        if(user === null){
            ...
            const mailOptions = {
                ...
                to: `${user.local.email}`,
                subject: 'Reset Email',
                text: `https://nwen304-group-host.herokuapp.com/resetPassword/${user._id}/${user.resetPasswordToken}`
            };
    ...
    app.get('/api/items/id/:id', async (req, res) => {
        var { id } = req.params;
        const item = await itemRequests.getItemById(ObjectId(id));
        res.json(item);
    })

    app.get('/api/users/id/:id', async (req, res) => {
        var { id } = req.params;
        const user = await userRequests.getUserById(ObjectId(id));
        res.json(user);
    })

    app.get('/api/items/update/:id', async (req, res) => {
        var { id } = req.params;
        const item = await itemRequests.updateItem(ObjectId(id));
        res.json(item);
    })
